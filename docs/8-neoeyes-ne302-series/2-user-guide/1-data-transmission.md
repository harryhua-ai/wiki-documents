...
NE302 以 RTSP 拉流作为对接第三方 NVR / VMS 的路径：

1. 在 NVR / VMS 中选择「手动添加设备」或「自定义 RTSP / 网络摄像头」；
2. 粘贴**视频流设置**页面生成的视频流 URL；启用了摘要认证时，填写对应的用户名和密码；
3. 保存后确认 NVR / VMS 持续显示画面。

NE302 控制台提供的通道为 MQTT、Webhook、RTSP、RTMP，不包含 ONVIF，因此接入采用手动添加 RTSP 流的方式。Frigate 等具体第三方 NVR / VMS 产品的兼容性不在 CamThink 官方验证记录内，请按上述步骤自行验证画面与稳定性；采购决策依赖特定 VMS 的官方兼容承诺时，先通过销售渠道确认。
